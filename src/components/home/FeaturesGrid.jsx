      ...
      <div className="max-w-6xl mx-auto px-6">
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature, index) => (
            <div key={index} className="bg-softBg rounded-xl p-6 hover:shadow-lg transition-shadow duration-300">
              <h3 className="text-lg font-bold text-textDark mb-3">{feature.title}</h3>
              <p className="text-textMuted mb-4">{feature.description}</p>
              <a 
                href="#more"
                className="inline-flex items-center gap-1 text-primary font-medium hover:text-primaryDark transition text-sm"
              >
                {feature.link}
