    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
        {[
          ...
          <div key={index} className="bg-white rounded-xl p-6 sm:p-8 shadow-lg border border-gray-100 hover:border-primary/20 transition-all duration-300 hover:shadow-xl">
            <div className="mb-6">
              <h3 className="text-lg sm:text-xl font-bold text-textDark mb-3">{tariff.title}</h3>
              <p className="text-textMuted text-sm sm:text-base">{tariff.description}</p>
            </div>

            <ul className="space-y-3 mb-8">
              {tariff.features.map((feature, idx) => (
                <li key={idx} className="flex items-center gap-3 text-textDark text-sm sm:text-base">
                  <svg className="w-4 sm:w-5 h-4 sm:h-5 text-primary flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
