  ...
  switch (step) {
    ...
    case 2: // StepIncome
      if (["Condominium", "MultiFamilyBuilding"].includes(data.propertyType)) {
        ...
        if (
          !data.income?.capitalizationRate ||
          data.income.capitalizationRate <= 0
        ) {
          errors.capitalizationRate =
            "Kapitalisierungszinssatz muss größer als 0 sein";
        }
      }
      break;
...
export function isCostValid(data) {
  ...
  if (data.cost.grossFloorArea > 0 || data.cost.standardConstructionCost > 0) {
    return (
      data.cost.grossFloorArea > 0 && data.cost.standardConstructionCost > 0
    );
  }
